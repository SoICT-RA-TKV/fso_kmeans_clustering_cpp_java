...
public class Main {
	...
	public static Point[] tapDiem; // tapDiem[i] = diem thu i(0 <= i <= n-1)
	...
	public static void nhapDuLieu() throws IOException {
		...
		tapDiem = new Point[n];
		for (int i = 0; i < n; i++)
			tapDiem[i] = new Point(sc.nextDouble(), sc.nextDouble());
	...
	public static void xuatHinh() throws IOException {
		...
		for (Point diem : tapDiem)
			fw.write(diem.toDecimalString() + "\n");
		fw.write("\n");

		for (int z = 0; z < k; z++)
			for (Point p : tapCluster[z].pointOfCluster) {
		...
		for (int i = 0; i < k; i++)
			for (Integer source : tapCluster[i].idPointOfCluster) {
				...
				while (true) {
					...
					if (mark[destination] != i)
						break;
				}
				if (destination == -1)
					System.out.println("BUGSS !");
				else {
	...
	public static boolean phanCum() {
		...
		while (true) {
			...
			for (int i = 0; i < n; i++) {
				...
				// -----------------------------------------------
				kCachMin = tapDiem[i].distance(tapCluster[0].getCentroid());
				indexOfCluster = 0;

				for (int j = 1; j < k; j++) {
					double kCach = tapDiem[i].distance(tapCluster[j].getCentroid());
					if (kCach < kCachMin) {
						kCachMin = kCach;
						indexOfCluster = j;
					}
				...
				while (tapCluster[indexOfCluster].getSize() > maxPoint) {
					...
					for (Integer p : tapCluster[indexOfCluster].idPointOfCluster)
						for (int z = 0; z < k; z++)
							if (mark[z] != nMark)
								if (tapDiem[p].distance(tapCluster[z].getCentroid()) < kcMin) {
									kcMin = tapDiem[p].distance(tapCluster[z].getCentroid());
									movePoint = p;
									destination = z;
								}
					if (destination == -1) {
		...
		// ------------------------------------------
		for (int z = 0; z < k; z++)
			if ((tapCluster[z].maxDistance() > maxRadius + EPS) || (tapCluster[z].getSize() > maxPoint))
				return false;
